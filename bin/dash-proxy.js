...
var server = http.createServer(function(req, res) {
  ...
  if (req.method == 'POST' && search_check == null ) {
     console.log("_time=\"" + epoch + "\" clientIp=\"" + req.connection.remoteAddress.replace(/^[:f]+/, '') + "\" proxy=\"" + req.headers.host + "\" message=\"Emergency, paging Dr. Beat! Someone is using a POST!!!\" url=\"" + req.url + "\"");
     return;
  }
  ...
  console.log("_time=\"" + epoch + "\" clientIp=\"" + req.connection.remoteAddress.replace(/^[:f]+/, '') + "\" proxy=\"" + req.headers.host + "\" url=\"" + req.url + "\"" );
  proxy.web(req, res);
});
